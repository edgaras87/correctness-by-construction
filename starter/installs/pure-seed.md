...
sed -i -e "/<!-- STEPS-BEGIN/r "<(echo; sed -n '/^## Step/,$p' \
    ...
    PLAN.md
sed -i "s|<playbook> v<N> at <handbook or concept commit>|cbc-run-pure v2 at $bundle_pin|" \
    PLAN.md
git add PLAN.md
git commit -m "chore: seed — steps into PLAN, cbc-run-pure v2 @ $bundle_pin"
```
...
- PLAN's STEPS region holds the pure variant's sequence —
  ...
  from:" comment names cbc-run-pure v2 at the bundle pin. No
