...
1. A standing rule into PLAN, above the steps: each step starts on
   its own branch cut from main, and ends after its gate closes by
   fast-forward merge into main, on the reviewer's word. A restarted
   step keeps its old branch renamed and cuts a new one.

2. A tracked .claude/settings.json holding one permission rule, so
   the harness stops every commit at a prompt I answer:

   { "permissions": { "ask": ["Bash(git commit *)"] } }

3. A file CLAUDE.local.md at the repository root, mine and untracked
   — add it to .gitignore beside the other machine-local entries.
   Its whole content, verbatim:
